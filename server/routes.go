...
func llmHandler(c echo.Context) error {
	...
	if !exists {
		return c.JSON(http.StatusInternalServerError, "LLM model not found")
	}
	fmt.Println(model)
	...
	s, err := llm(model, SYSTEM, PROMPT+textToTranslate)
	if err != nil {
...
func llm(model string, system string, prompt string) (string, error) {
